...
public class Player : MonoBehaviour {
	...
	void Start () {
		...
		//Sample Notification 
		StartCoroutine (notification("¡Bienvenido a Pompas/3"));
...
//Made this code simpler, calls rotate function
	...
	void counter(){
		if(canvas != null)
			countText.text = "Tanteo: " + counts.ToString ();
			streakSlider.value = counts;
	}
	void score(){
		if(canvas != null)
			scoreText.text = "Puntos: " + gamePoints.ToString ();
	}
	...
	void highStreakText(){
		...
		streakText.text = "Racha: " + counts.ToString () + "/" + highStreak.ToString();
		if(canvas != null)
			countText.text = "Tanteo: " + counts.ToString ();
	}
//
...
//			//-----------------------------------	
	...
	void healthBarInitial(){
		...
		if (health <= 0) {
            ...
            else if (gamePoints > PlayerPrefs.GetInt("score3"))
            {
                PlayerPrefs.SetInt("score3", gamePoints);
            }
            else if (gamePoints > PlayerPrefs.GetInt("score4"))
            {
                PlayerPrefs.SetInt("score4", gamePoints);
            }
	...
	private int multiplier(){
		...
		if(counts < 11){
			...
			if((counts -1)%10 == 0){
				...
				StopCoroutine ("notification");
				StartCoroutine ("notification","x" + num.ToString() + "¡Multiplicador alcanzado!");
			}
		} 
